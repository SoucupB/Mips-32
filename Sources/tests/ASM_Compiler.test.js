...
test('Check Compiler checker v13 (Full Program).', (t) => { // rsp register is not pushed onto the stack and therefor its value is lost
  const program = new Program('int fibboRecursive(int n){if(n<2){return 1;}return fibboRecursive(n-1)+fibboRecursive(n-2);}void main(){int b=fibboRecursive(5);}') // rsp should have been pushed into the stack
  let chomp = program.chomp();
  t.equal(chomp.isInvalid(), false, 'returns');
  let programCompiler = new Compiler(null);
  let asmBlock = programCompiler.compileProgram(chomp);
  // console.log(asmBlock.toString())
  // asmBlock.run()
  // console.log(asmBlock.runner.printPointerBytes(256))
